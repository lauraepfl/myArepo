...
/**begin modified by MP*/


#ifdef BARROS
extern void F90gravity( double *z,  double *g, double *scale_l, double zmid);
const long double pc = 3.086e18;
...
static void gravity_external_get_force(double pos[3], int type, MyIDType ID, double acc[3], double *pot, int *flag_set)
{
  double g;
  for(int k = 0; k < 3; k++)
    acc[k] = 1e-14/(All.UnitLength_in_cm/pow(All.UnitTime_in_s,2.0));
#ifdef BARROS
/*
    extern void F90gravity(long double *z, long double *g);
    const long double pc = 3.086e18;
*/
  for(int k = 0; k < 3; k++){
    F90gravity(&pos[3], &g, &All.UnitLength_in_cm, 0.5*All.BoxSize );
    acc[k] = g/(All.UnitLength_in_cm/pow(All.UnitTime_in_s,2.0));
